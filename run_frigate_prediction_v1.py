...
from snapy import kIDN, kIPR, kIV1, kIV2, kIV3, kICY
...
def load_ecmwf_input(input_file: str, index: int = 0,
    ...
    for name, var in module.named_buffers(recurse=True):
        data[name] = var[index].to(device).to(torch.double)

    for name, var in module.named_parameters(recurse=True):
        data[name] = var[index].to(device).to(torch.double)

    block_vars = {}
    block_vars["hydro_w"] = data["hydro_w"][:kICY]
    return block_vars
...
def run_simulation_one_day(block: MeshBlock, 
    ...
    for chunk in range(4):
        block_vars, current_time = run_simulation(block, block_vars, current_time, 216.)
...
def main():
    ...
    block, device = create_block(args.config)
    ...
    print(block_vars["hydro_w"][kIDN].min(), block_vars["hydro_w"][kIDN].max())
    print(block_vars["hydro_w"][kIPR].min(), block_vars["hydro_w"][kIPR].max())
    ...
    block.make_outputs(block_vars, current_time)
    block_vars, current_time = run_simulation(block, block_vars, current_time, 100.)
    ...
    for day in range(days):
        block_vars, current_time = run_simulation_one_day(block, block_vars, current_time,
                                                          args.input)
        block, block_vars = refine_simulation(
    ...
    block_vars, current_time = run_simulation(block, block_vars, current_time, duration)
