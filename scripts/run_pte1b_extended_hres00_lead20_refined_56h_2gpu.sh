...
TOPO_SOURCE_RUN="${TOPO_SOURCE_RUN:-$RUNS_DIR/pte1b-extended-topography}"
...
RUN_DIR="$RUNS_DIR/${REGION_ID}-${TARGET_DATE}-hres00-lead20"
FORECAST_INPUT_DIR="$RUN_DIR/forecast_input"
TENSOR_DIR="$FORECAST_INPUT_DIR/regridded_${REGION_ID}_${TARGET_YMD}_${FORECAST_CYCLE}_tensors"
RESTART_BUNDLE="$TENSOR_DIR/regridded_${REGION_ID}_${TARGET_YMD}_${FORECAST_CYCLE}.restart"
TOPOGRAPHY_DIR="$RUN_DIR/topography/products"
RAW_TOPOGRAPHY_DIR="$RUN_DIR/topography/raw/$REGION_ID"
SMOKE_DIR="$RUN_DIR/forecast_lowres_hres_2gpu_smoke"
OUTPUT_DIR="$RUN_DIR/forecast_refined_hres_hourly_56h_2gpu"
RUNTIME_RESTART="$OUTPUT_DIR/${REGION_ID}.final.restart"
REFINED_CONFIG="$OUTPUT_DIR/${REGION_ID}.refined.yaml"
COMPLETE_MARKER="$OUTPUT_DIR/run.complete"
SESSION_NAME="pte1b_extended_${TARGET_YMD}_00_lead20_hres_refined_56h_2gpu"
...
SOURCE_MERGED_TIF="$TOPO_SOURCE_RUN/topography/raw/$REGION_ID/${REGION_ID}_merged_10m.tif"
...
if [[ ! -f "$KML_FILE" ]]; then
  echo "$REGION_ID KML file not found: $KML_FILE" >&2
  exit 1
fi
...
"$PYTHON_BIN" - "$WORKSPACE" "$KML_FILE" "$RUN_DIR/${REGION_ID}.yaml" "$TARGET_DATE" "$REGION_ID" <<'PY'
...
region = load_region(region_kml=kml_file, location_id=region_id)
prepared = build_prepared_domain(region)
write_config(config_path, region, prepared, date=target_date, nb2=2, nb3=1)
PY

cp "$SOURCE_MERGED_TIF" "$RAW_TOPOGRAPHY_DIR/${REGION_ID}_merged_10m.tif"
if [[ -f "$SOURCE_TOPOGRAPHY_DIR/${REGION_ID}_topo_2p4km.pt" \
   && -f "$SOURCE_TOPOGRAPHY_DIR/${REGION_ID}_topo_1p2km.pt" \
   && -f "$SOURCE_TOPOGRAPHY_DIR/${REGION_ID}_topo_0p6km.pt" \
   && -f "$SOURCE_TOPOGRAPHY_DIR/${REGION_ID}_topo_0p3km.pt" ]]; then
  cp "$SOURCE_TOPOGRAPHY_DIR"/${REGION_ID}_topo_*.pt "$TOPOGRAPHY_DIR"/
else
...
if [[ "${SMOKE_ONLY:-0}" == 1 ]]; then
  echo "Smoke test completed successfully: $SMOKE_DIR/smoke.ok"
  exit 0
fi
